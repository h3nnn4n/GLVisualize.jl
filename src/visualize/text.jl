...
function visualize{S <: AbstractString}(text::Signal{S}, s::Style, customizations=visualize_default(text, s))
    ...
    const_lift(update_text, text, Signal(robj))
    robj
end 
function visualize(
...
function textedit_signals(inputs, background, text)
    ...
    const_lift(update_positions, text_sig, Signal(text), Signal(background[:style_index]))
    foldp(visualize_selection, 0:0, selection,    Signal(background[:style_index]))
